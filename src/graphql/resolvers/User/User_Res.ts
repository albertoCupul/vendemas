...
export class UserResolver {
  ...
  async QueryUserByName(@Arg("name") name: string): Promise<UserResponse[]> {
    const user: DocumentType<UserMain>[] = await UserModel.find()
      .findByName(name)
      .orFail()
      .exec();
  ...
  async AddUser(
    @Args() { name, first, second, phone, PerfilID }: InputUserName
    ...
    if (perfil) {
      ...
      user = await UserModel.findById(user._id.valueOf())
        .populate("idAccess")
        .populate("idPerfil")
        .exec();
  ...
  async UpdateUserMain(
    @Args() { id, name, first, second, phone }: UpdateUserName
  ): Promise<UserResponse> {
  ...
  @Mutation(returns => Boolean)
  async wz3_createUserAdmin(): Promise<Boolean>{
    try {
      let response: boolean = false;      
      let user = new UserModel();
      ...
      if (perfil) {
        const existAdminUser = await UserModel.findOne({ name: "Administrador" });
        if (!existAdminUser){
