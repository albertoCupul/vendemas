...
import {ModuleMain, 
        ...
        UpdateModule, 
        ModuleResponse,
        ModuleModel} from  '../../../typegoose/schemas/User/userModules'
        
import {moduleResponseFix, moduleArrayResponseFix, existInDB} from '../../../typegoose/schemas/User/middleware'
...
export class UserModulesResolver{
  ...
  async QueryModuleList ():Promise<ModuleResponse[]>{
    const moduleData: Array<ModuleMain> = await ModuleModel.find({}).orFail().exec()
  ...
  async AddModule(@Args() {name} : InputModule):Promise<ModuleResponse>{
    try{
        ...
        if (!alreadyExist){
            ...
            module = await ModuleModel.findById(module._id.valueOf()).orFail().exec()            
  ...
  async AddModuleList():Promise<Boolean>{
    try{
        listModules.forEach(async (elemento) => {
            const alreadyExist : boolean = await existInDB(elemento)            
            if (!alreadyExist){
                let module = new ModuleModel()
