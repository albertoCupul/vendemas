...
import {PerfilMain, 
        ...
        UpdatePerfil, 
        PerfilResponse,
        PerfilModel} from  '../../../typegoose/schemas/User/userPerfilSchema'
        
import {ModuleModel, ModuleMain} from '../../../typegoose/schemas/User/userModules'
        
import {perfilResponseFix, perfilArrayResponseFix, existInDBPerfil, modelExistInRule} from '../../../typegoose/schemas/User/middleware'
...
export class UserPerfilResolver{
  ...
  async QueryPerfilById (@Arg('id') id:string):Promise<PerfilResponse>{
    const perfilData = await PerfilModel.findById(id).populate('idModules').orFail().exec()
  ...
  async QueryPerfilList ():Promise<PerfilResponse[]>{
    const perfilData: Array<PerfilMain> = await PerfilModel.find({}).populate('idModules').orFail().exec()
    const response: Array<PerfilResponse> = perfilArrayResponseFix(perfilData)  
    return response
  }
  
  @Query(returns => Boolean)
  async QueryModuleInPerfil (@Arg('idPerfil') idPerfil:string, @Arg('idModule') idModule:string):Promise<Boolean>{
        ...
        listPerfil.perfil.forEach(async (elemento) => {
            ...
            await Promise.all(modelos).then(async (arrayModelos) => {
                ...
                if (!alreadyExist){
                    ...
                    await perfil.CreateNew({namePerfil, arrayModelos})
