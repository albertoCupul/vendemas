...
import {PerfilResponseFix, ClientResponseFix, ClientArrayResponseFix} from '../../../typegoose/schemas/Client/middleware'
...
export class ClientResolver {
  ...
  @Query(returns => [ClientResponse])
  async QueryClientByName(@Arg('name') name:string) : Promise<ClientResponse[]>{
    let response : Array<ClientResponse>
    const client = await ClientModel.find({fullName:{$regex: name, $options: 'i'}}).populate('idPerfil').exec()
    response = await ClientArrayResponseFix(client)
    return response
  }
  
  @Mutation(returns => ClientResponse)
  async AddClient (@Args() {name, first, second, address, references, phone, 
  rfc, credit, PerfilID} : InputClient) : Promise<ClientResponse> {
    let response : ClientResponse
    let client = new ClientModel()
    const perfil = await PerfilModel.findById(PerfilID).exec()   
    if (perfil){
      await client.CreateAndUpdateClient({name, first, second, address, references, phone, 
        rfc, credit, perfil})
      client = await ClientModel.findById(client._id.valueOf()).populate('idPerfil')
      response = await ClientResponseFix(client)      
    }
    return response
  }
  ...
  @Mutation(returns => ClientResponse)
  async UpdateClient (@Args() {id, name, first, second, address, references, phone, 
  rfc, credit, PerfilID} : UpdateClient) : Promise<ClientResponse> {
    let response : ClientResponse
  ...
  @Mutation(returns => Boolean)
  async AddClienteGeneral ():Promise<Boolean>{
    const client = new ClientModel()
    let response : boolean = false
    let name:string ,first:string,second : string, address:string, references:string, 
    phone:string, rfc:string, credit:number
    const perfil = await PerfilModel.findOne({name: ClienteGeneral.perfilName}).exec()
    if (perfil){
      ...
      const client = new ClientModel()
      client.CreateAndUpdateClient({name, first, second, address, references, phone, 
      rfc, credit, perfil})
      response = true
