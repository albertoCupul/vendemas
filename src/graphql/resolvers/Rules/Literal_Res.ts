...
import {LiteralModel, LiteralMain, InputLiteral, UpdateLiteral, LiteralResult} from '../../../typegoose/schemas/Rules/LiteralSchema'
import {LiteralArrayResponseFix, literalResponseFix, LiteralExistInRule} from '../../../typegoose/schemas/Rules/middleware'

@Resolver(LiteralMain)
export class LiteralResolver{
  
  @Query(returns => [LiteralResult])
  async QueryLiteralByName(@Arg('name') name: string) : Promise<LiteralResult[]>{
      const literals:DocumentType<LiteralMain>[] = await LiteralModel.find().findByName(name)
  ...
  async AddLiteral (@Args() {name, value, editable} : InputLiteral ): Promise<LiteralResult>{
  ...
  @Mutation(returns => LiteralResult)
  async UpdateLiteral (@Args() {id, name, value, editable}: UpdateLiteral ): Promise<LiteralResult>{
